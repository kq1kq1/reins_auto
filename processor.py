...
def _backend() -> str:
    return _STORAGE.get("backend", "excel")
...
def load_log(db_path: str) -> pd.DataFrame:
    if _backend() == "sheets":
        import sheets_backend
        return sheets_backend.read_sheet(_STORAGE, SHEET_LOG, LOG_COLUMNS)
    if not Path(db_path).exists():
...
def load_state(state_path: str) -> dict:
    ...
    if _backend() == "sheets":
        import sheets_backend
        return sheets_backend.read_state(_STORAGE)
    p = Path(state_path)
    if not p.exists():
        return {}
    try:
...
def save_state(state_path: str, state: dict) -> None:
    ...
    if _backend() == "sheets":
        import sheets_backend
        sheets_backend.write_state(_STORAGE, state)
        return
    try:
