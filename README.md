...
現在は **Googleスプレッドシート**が正。あわせて **Supabase(PostgreSQL)** へ写しを保存している
（`config.json` の `postgres.enabled`）。将来的にSupabaseへ一本化する予定。
...
| `スプレッドシートに接続できません` | ネット接続を確認して再実行。この時点では何も書き換えていないので再実行して安全 |
| `DBを読み込めませんでした` | 同上。読めない状態で続行すると全件が「新規」扱いになるため、意図的に中止している |
| `保存を中止しました（データが異常に減っています）` | 通信不良でDBを読めていない可能性。再実行で直る。取得データは `exports/緊急退避_*` にCSVで退避済み |
| `条件を選択できませんでした` | その条件だけスキップして次に進む。ログで対象条件を確認し、必要なら再実行 |

### データ保護の仕組み

スプレッドシートは「丸ごと置き換え」で保存するため、通信断で全消しにならないよう
3段の安全装置を入れている。

1. **読み込み失敗を空データにしない** … 読めなかったら例外を投げて中止（`DbLoadError`）
2. **通信エラーは自動リトライ** … 5秒→15秒→30秒で3回まで再試行
3. **保存前の件数チェック** … 読み込み時の半分未満になる保存は中止し、CSVに退避（`DbWriteError`）

書き込み順も「広げる→上書き→切り詰める」にしてあり、
成功するまで既存データを消さない（`clear()`してから書かない）。

万一データが消えた場合は、スプレッドシートの **ファイル → 版履歴** から復元できる。
